...
TOTAL_TIMESTEPS = 10_000_000  # Set high (10M). The agent determines the pace, not the clock.

# 🎯 PROMOTION THRESHOLDS (Avg Reward over 100 episodes)
# Level 0 -> 1: Needs > 3.30 (Must hit stationary target perfectly)
# Level 1 -> 2: Needs > 3.0 (Must hit linearly moving target consistently)
...
# 3. Setup Callbacks
# A. Curriculum Manager (Checks every 10k steps)
curriculum_callback = CurriculumCallback(check_freq=10000, log_dir=LOG_DIR)

# B. Safety Checkpoint (Saves every 200k steps)
# Files will be named: "tank_backup_200000_steps.zip", etc.
checkpoint_callback = CheckpointCallback(
    save_freq=200000, 
    save_path=CHECKPOINT_DIR, 
    name_prefix="tank_backup"
)
...
print(f"Backups: {CHECKPOINT_DIR} (Every 200k steps)")
